...
envData3 = envData %>% select(depth, lat, BO22_ph, BO22_parmean)

vif = diag(solve(cor(envData3)))
vif
#all lower than 10 when remove nitrate
...
model = envData3 %>% dplyr::select("pH" = BO22_ph, "PAR" = BO22_parmean, "Depth" = depth, "Latitude" = lat)

pastDbrda = dbrda(pastMa ~ pH + Depth + Latitude + PAR, model)
