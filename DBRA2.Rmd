...
present = list_layers(datasets) %>% dplyr::select(dataset_code, layer_code, name, units, description, contains("cellsize"), version) %>% filter(version == 2.2) %>% filter(layer_code %in% c("BO22_parmean", "BO22_ph", "BO22_dissoxmean_bdmean", "BO22_lightbotmean_bdmean", "BO22_tempmean_ss", "BO22_salinitymean_bdmean", "BO22_nitratemean_bdmean"))

envVar = load_layers(present$layer_code)

#Import coordinates of sites
popData=read.csv("data_files/poritesastreoidesMetaData_clonesremoved.csv") %>% select("sample" = tube_id, "region" = region, "depth" = collection_depth_m, "lat" = site_lat, "lon" = site_long)

envData = data.frame(popData, raster::extract(envVar, popData[,5:4]))
# write_excel_csv(envData, file= 'Sites_w_Env_Data.csv')

#visualizing to then remove variables that had significant colinearity >0.7, or that had vif >10
library(Hmisc)
library(reshape2)
...
EnvPlotCor2 = ggplot(corDataBind) +
...
#issue when including temp and trying to run vif, removing since barely varies anyways (by ~0.2C) and re-running
#when run ph, parmean, salinity mean, and nitrate mean, vifs are crazy high?? like in the 20-30 thousands
envData2 = envData %>% select(BO22_ph, BO22_parmean, BO22_salinitymean_bdmean, BO22_nitratemean_bdmean)

#try diff
envData3 = envData %>% select(BO22_ph, BO22_parmean, BO22_salinitymean_bdmean, BO22_dissoxmean_bdmean)

corData2 = rcorr(as.matrix(envData2), type = "pearson")
corDataFlat2 = melt(corData2$r, value.name = "r")
pDataFlat2 = melt(corData2$P, value.name = "p")
corDataBind2 = corDataFlat2 %>% left_join(pDataFlat2, by = c("Var1","Var2"))

EnvPlotCor2 = ggplot(corDataBind2) +
   geom_tile(aes(x = Var1, y = Var2, fill = r)) +
   scale_fill_gradient2(low = "#3B9AB2FF", high = "#F21A00FF") +
   geom_text(data = filter(corDataBind2, r >= 0.7, p < 0.05),aes(x = Var1,    y = Var2, label = round(r, 2))) +
   geom_text(data = filter(corDataBind2, r <= -0.7, p < 0.05),aes(x =        Var1, y = Var2, label = round(r, 2))) +
   theme_bw() +
   theme(axis.text.x = element_text(angle = 90))
...
#looks better when temp is removed, so moving forward with:
#BO22_ph, BO22_parmean, BO22_salinitymean_bdmean, BO22_dissoxmean_bdmean
vif = diag(solve(cor(envData3)))
vif

pastMa = as.matrix(read.table("data_files/pastNoClones.ibsMat"))

rda1 = dbrda(pastMa ~ ., data = envData3)
RsquareAdj(rda1)
anova.cca(rda1)

rda0 = dbrda(pastMa ~ 1, data = envData3)
rda1 = dbrda(pastMa ~ ., data = envData3)
...
model = envData3 %>% dplyr::select("pH" = BO22_ph, "PAR" = BO22_parmean, "Salinity" = BO22_salinitymean_bdmean, "DO" = BO22_dissoxmean_bdmean)

pastDbrda = dbrda(pastMa ~ pH + DO + Salinity + PAR, model)

pastRdaVar = round(pastDbrda$CA$eig/sum(pastDbrda$CA$eig)*100, 1)
head(pastRdaVar)

# pastI2P = read.csv("../data/stephanocoeniaMetaData.csv")[-c(66,68,164,166,209,211),] %>% dplyr::select("sample" = tubeID, "pop" = region, "depth" = depthZone)
# 
