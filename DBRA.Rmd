...
```{r, env data}
#packages: sdmpredictors, Hmisc, reshape2
...
#filtering things out, can't do this until I look at covariances
#envData=envData%>%dplyr::select(-BO22_dissoxmean_ss, -BO22_carbonphytomean_ss, -BO22_nitratemean_ss, -BO22_chlomean_ss, -BO22_silicatemean_ss, -BO22_ppmean_ss, -BO22_ironmean_ss, -BO22_parmean)
 # corData = cor(envData[,c(4:ncol(envData))])
 # corMelt = melt(corData)

  #envSummary=envData %>% group_by(region, depthZone, lat, lon) %>% summarise(temperature=min(temperature), chlorophyll=min(chlorophyll), currents = min(currents), 
  #nitrate = min(nitrate), phosphate=min(phosphate), prod=min(prod), salinity=min(salinity), silicate=min(silicate), iron=min(iron))
