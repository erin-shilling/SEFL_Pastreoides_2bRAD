...
```{r, relate map}
pastFam = pastRelate2 
# pastFam$popDepth.a = factor(pastFam$popDepth.a)
# pastFam$popDepth.b = factor(pastFam$popDepth.b)
# pastFam$popDepth.a = factor(pastFam$popDepth.a, levels = levels(pastFam$popDepth.a)[c(8, 7, 2, 1, 6, 5, 4, 3)])
# pastFam$popDepth.b = factor(pastFam$popDepth.b, levels = levels(pastFam$popDepth.b)[c(8, 7, 2, 1, 6, 5, 4, 3)])
...
# pastFam = pastFam %>% arrange(popDepth.a)
pastFam$sample.a = factor(pastFam$sample.a)
pastFam$sample.a = factor(pastFam$sample.a, levels = unique(pastFam$sample.a))
pastFam$sample.a = factor(pastFam$sample.a, levels = c("SFK220", levels(pastFam$sample.a)))
...
pastMat <- pastMat[,sampleOrder] %>%
  .[sampleOrder,]
diag(pastMat)<-NA
# pastMat[upper.tri(pastMat)] <- NA
pastMat <- as.data.frame(pastMat)
pastMat$sample.a = factor(colnames(pastMat))
# popData = read.csv("data_files/poritesastreoidesMetaData_clonesremoved.csv") %>% select("sample.a" = tube_id, "region" = region) %>% mutate(popDepth = paste(region, depth, sep = "\\n"))
pastMat = pastMat %>% left_join(popData)
pastMat$sample.a = factor(pastMat$sample.a, levels = unique(sampleOrder))
# pastFam = melt(pastMat, id.vars = c("sample.a", "region", "depth", "popDepth"), value.name = "relate", variable.name = "sample.b", na.rm = FALSE)
# pastFam = pastFam %>% add_row(sample.a = NA, region = "Region", depth = "Depth Zone", popDepth = NA, sample.b = NA, relate = NA) %>% add_row(sample.a = NA, region = NA, depth = " ", popDepth = NA, sample.b = NA, relate = NA)
...
popData$pop = factor(popData$pop, levels = levels(popData_wclones$pop)[c(4, 2, 5, 1, 3)])
...
zooxMaps = NULL
for(i in zooxLst){
  zoox = rbind(zooxMaps, data.frame(t(i)))
}
...
#bind it to metadata
dfZoox = popData_wclones %>% left_join(zooxProp)
