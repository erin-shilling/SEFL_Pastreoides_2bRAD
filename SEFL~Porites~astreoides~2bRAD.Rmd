...
TO BE FINISHED
Outliers
On 10/21/21 running bayeScEnv with depth as a factor so I do not have a qval_g that has been generated yet; later can also add other factors like chlorophyll, current, etc if want
```{r, outlier}
...
##try to run this next line it says object qval_g not found
##Haven't run this in koko yet bc don't currently have environmental factors, figure generated currently is only the chunk above
bayeScEnv = read.table("C:/Users/erin_/Documents/GitHub/SEFL_Pastreoides_2bRAD/data_files/NOFILEYET", header=T) %>% filter(qval_g < 0.05) %>% mutate(loc = rownames(.), depthOut = 1) %>% select(loc, depthOut)
...
![](C:/Users/erin_/Documents/GitHub/SEFL_Pastreoides_2bRAD/figures/depth_mantelPlot.png)
...
NEED TO CHECK THIS WHERE IT SAYS SAMPLES ARE MISSING?
