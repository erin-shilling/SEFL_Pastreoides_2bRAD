...
```{r, Pairwise Fst}
#packages: StAMPP, gdata, reshape2
...
Is it just calculating stuff for subsequent chunks?
```{r, popStats}
...
This shows the exact same thing as the chunk before it??
```{r, minor allele frequencies/densities}
mafDensPlotA = ggplot(data = seflpastAF, aes(x = minFreq, group = pop, color = region)) + 
...
ggsave("C:/Users/erin_/Documents/GitHub/SEFL_Pastreoides_2bRAD/figures/mafDensPlot.pdf", plot = mafPlot, width = 14, height = 8, units = "cm", dpi = 300)
ggsave("C:/Users/erin_/Documents/GitHub/SEFL_Pastreoides_2bRAD/figures/mafDensPlot.png", plot = mafPlot, width = 14, height = 8, units = "cm", dpi = 300)

# mafDensPlot

```

##not relevant bc didn't sample across depths?
Have not tried to run yet
```{r, linear regression of minor allele frequencies by depth}
mafLm = summary(lm(data = seflpastAF, minFreq ~ avgDepthM))
...
shalInbred = read.delim("C:/Users/erin_/Documents/GitHub/SEFL_Pastreoides_2bRAD/data_files/seflpastInbredAlleleFreq.txt")
bams = read.delim("C:/Users/erin_/Documents/GitHub/SEFL_Pastreoides_2bRAD/data_files/bamsNoClones", header = FALSE) %>% mutate(a = c(1:36))
pops = read.csv("C:/Users/erin_/Documents/GitHub/SEFL_Pastreoides_2bRAD/data_files/poritesastreoidesMetaData_clonesremoved.csv") %>% select("Region" = region) %>% mutate(a = c(1:36), pop = paste(Region)) %>% mutate(across(where(is.character), str_remove_all, pattern = fixed("'")))  %>% 
  mutate(across(where(is.character), str_remove_all, pattern = fixed(" ")))
# Reads in population data
shalBams = bams %>% left_join(pops) %>% left_join(shalInbred)
%>% select(-a)
...
Not significant, not making figure for it
...
MAY WANT TO REMOVE FIGURE FOR K=7
ADMIXTURE plot
