...
Calculating values for later plotting
```{r, popStats}
# packages: tidyverse, rstatix
#rstatix package for Tukey post hoc
#Read in population data
popData = read.csv("data_files/poritesastreoidesMetaData_clonesremoved.csv") %>% select("sample" = tube_id, "Population" = region)
...
hetAllpwc
#only Jup and FLL sig diff from each other
...
hetPlotAll = ggplot(data = subset(hetMelt, subset = hetMelt$type == "All"), aes(x = Population, y = heterozygosity, fill = Population)) +
  ...
  theme_bw() +
  hetTheme + 
  stat_compare_means(comparisons = hetAll_comp, label = "p.signif", label.y = c(0.0036))
...
hetPlotSnps = ggplot(data = subset(hetMelt, subset = hetMelt$type == "SNPs"),aes(x = Population, y = heterozygosity, fill = Population)) +
  geom_point(aes(color = Population),shape = 21, position = position_jitterdodge(seed = 1, dodge.width = 1), size = 2, alpha = 0.7) +
  geom_boxplot(size = 0.5, alpha = 0.7, position = dodge, color = "gray35", outlier.shape = NA) +
  coord_cartesian(ylim = c(0.25, 0.46)) +
  xlab("Population") +
  ylab("Heterozygosity") +
  ggtitle("SNPs") +
  scale_fill_manual(values = flPal) +
  ...
  theme_bw() +
  hetTheme +
  stat_compare_means(comparisons = hetSnp_comp, label = "p.signif", label.y = c(0.44))
...
inbreedingPlot = ggplot(data = het, aes(x = Population, y = inbreed, fill = Population)) +
  ...
  theme_bw() +
  hetTheme +
  stat_compare_means(comparisons = inbreed_comp, label = "p.signif", label.y = c(0.164, 0.148, 0.132))
...
=======
ggsave("C:/Users/erin_/Documents/GitHub/SEFL_Pastreoides_2bRAD/figures/heterozygosityPlot_pval.png", plot = hetPlots, width = 22, height = 16, units = "cm", dpi = 300)
ggsave("C:/Users/erin_/Documents/GitHub/SEFL_Pastreoides_2bRAD/figures/heterozygosityPlot_pval.pdf", plot = hetPlots, width = 22, height = 16, units = "cm", dpi = 300)
>>>>>>> Stashed changes
```
