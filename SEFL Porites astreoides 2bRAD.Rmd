...
knitr::opts_knit$set(root.dir = 'C:/Users/erin_/Documents/GitHub/SEFL_Pastreoides_2bRAD')
```
...
pastPcoaPlotA = ggplot(pastPCoA, aes(x = PCo1, y = PCo2, color = pop, fill = pop)) +
  ...
  geom_point(aes(x = mean.x, y = mean.y), size = 5, color = "black", shape = 21) + #population centroids indicated by triangles
...
snpFst$region = factor(snpFst$region, levels = levels(snpFst$region)[c(4, 1, 2, 3, 5)])

snpFst$region2 = snpFst$Pop2
snpFst$region2 = factor(gsub("\\n.*", "", snpFst$region2))
snpFst$region2 = factor(snpFst$region2, levels = levels(snpFst$region2)[c(4, 1, 2, 3, 5)])
...
###mess with next 4 lines to fix sizes of boxes that region names are in
  ...
  scale_color_manual(values = flPal[], guide = NULL) +
  scale_fill_gradient(low = "white", high = "#BF5E49", limit = c(0, 0.07), space = "Lab", name = expression(paste(italic("F")[ST])), na.value = "white",  guide = "colourbar")+
